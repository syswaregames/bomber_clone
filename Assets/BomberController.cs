...
public class BomberController : MonoBehaviour
{
    ...
    public float mountingLerpDuration = 0.5f;
    public float mountingJumpHeight = 2f;
    ...
    public void SetWalkDirectionFromExternalMounting(WalkDirection newWalkDirection, bool flipX)
    {
        walkDirection = newWalkDirection;
        animator.SetInteger("walkDirection", (int)newWalkDirection);
        if (invertFlip)
    ...
    public void StartMounting(MountableController mountable)
    {
        if (mounting)
            return;

        StartCoroutine(StartMountingCo(mountable));
    }
    IEnumerator StartMountingCo(MountableController mountable)
    {
        mounting = true;
        playable = false;
    ...
    public void OnCollisionEnter2D(Collision2D col)
    {
        ...
        {
            //Physics2D.IgnoreCollision(collider, contact.collider);
    ...
    {
        if (hitboxLayerMask.Contains(col.gameObject.layer))
        {
            var egg = col.gameObject.GetComponentInParent<Egg>();
            if (egg is not null)
            {
                egg.BornNow(this);
            }
